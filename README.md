...
Néanmoins, notre observation des modèles sur Hugging Face a révélé la récurrence de séquences diverses, telles que GPTQ, AWQ, GGUF, GGUML, 3b, 7b, 13b, small, medium, large, base, finetuned, et d'autres encore.
...
Dans le cadre de cette étude, nous avons développé un script dédié pour analyser les stratégies de versionnement adoptées par les modèles en fonction de leur catégorie d'application. Le script aborde en premier lieu la complexité des noms de modèles, qui sont fréquemment constitués de plusieurs éléments (tokens), séparés soit par des tirets soit par des points. Nous avons identifié que les noms d comprennent à la fois le nom du modèle et sa version, et observé que le premier token représente invariablement le nom du modèle. Nous avons donc extrait la composante correspondant au token de version, situé immédiatement après le nom du modèle. Si des tokens subséquents se révèlent être des éléments intrinsèques du nom du modèle, ils sont alors assimilés à la version. Cette approche n'introduit pas de biais significatif, affectant de manière minime les tendances générales détectées dans les schémas de versionnement.

Les modèles présents une seule fois dans notre échantillon ont été exclus de l'analyse, partant du principe que ces occurrences uniques sont moins susceptibles de refléter des pratiques de versionnement systématiques, ce qui réduit notre capacité à tracer l'évolution des conventions de nommage des versions.

De plus, nous avons délibérément exclu les identifiants numériques de notre analyse. Cette décision est fondée sur la prévalence de tels identifiants dans les pratiques standard de versionnement sémantique en développement logiciel. Notre intérêt se portant sur l'identification de pratiques de versionnement plus atypiques et nuancées, les identifiants numériques sont jugés moins révélateurs dans ce contexte. Cette exclusion stratégique vise à orienter notre analyse vers l'identification de stratégies de versionnement distinctives, écartant ainsi les approches conventionnelles de versionnement sémantique.
